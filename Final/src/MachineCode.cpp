...
void MovMInstruction::output()
{
    ...
    {
        Converter11 converter;
        converter.imm = static_cast<unsigned int>(use_list[0]->getVal());
        fprintf(yyout, "#%f", converter.val);
    }
    else
        use_list[0]->output();
    fprintf(yyout, "\n");
    ...
    if (commonVar.size() > 0)
    {
        ...
        for (auto se : commonVar)
        {
            if (se->getType()->isArray())
            {
                if (((IdentifierSymbolEntry *)se)->isInitial() && !((IdentifierSymbolEntry *)se)->isAllZero()) // 数组有初始化
                {
                    ...
                    if (((ArrayType *)se->getType())->getBaseType()->isFloat())
                    {
                        ...
                        {
                            Converter12 converter;
                            converter.value = static_cast<float>(arrayValue[i]);
            ...
            else
            {
                ...
                {
                    Converter12 converter;
                    converter.value = (float)((IdentifierSymbolEntry *)se)->getValue();
                    fprintf(yyout, "\t.word %u\n", converter.num);                        
                }
            }
        }
